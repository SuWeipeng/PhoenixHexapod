...
//delay(3000);
//SSCSerial.println(F("Line#254P512T500"));
//delay(1000);
//SSCSerial.println(F("Line#254P512T500"));
...
#ifdef SSCSerial
  //SSCSerial.print(F("Check GP Enable: "));
  //SSCSerial.println(cbRead, DEC);
#endif        
  if (cbRead == 4)
    _fGPEnabled = true;  // starts off assuming that it is not enabled...
  else
    MSound (3, 100, 2200, 150, 2550, 280, 3500);
...
#ifdef cVoltagePin  
#ifndef CVADR1
#define CVADR1      2.2  // VD Resistor 1 - reduced as only need ratio... 30K and 10K
#define CVADR2      1  // VD Resistor 2
#endif
...
//------------------------------------------------------------------------------------------
#define cPwmDiv       991  //old 1059;991
#define cPFConst      592  //old 650 ;592 900*(1000/cPwmDiv)+cPFConst must always be 1500
...
{        
  ...
  if ((byte)pgm_read_byte(&cTarsLength[LegIndex])) {
	...
	//SSCSerial.print((wTarsSSCV-476)/2, DEC);
  }
#endif
#endif        
...
void ServoDriver::FreeServos(void)
{
  g_InputController.AllowControllerInterrupts(false);    // If on xbee on hserial tell hserial to not processess...
/*  SSCSerial.print("Line");
  for (byte LegIndex = 1; LegIndex < 25; LegIndex++) {
    
	SSCSerial.print("#");
    SSCSerial.print(LegIndex, DEC);
    SSCSerial.print("P512");
  }
...
void FindServoOffsets()
{
  ...
  while(!fExit) {
    if (fNew) {
	  ...
	  //SSCSerial.print((1500+asOffsets[sSN]+250-476)/2, DEC);
	  ...
	  //SSCSerial.print((1500+asOffsets[sSN]-476)/2, DEC);
      SSCSerial.println("T250");
      delay(250);
